...
export class CasaController {
  ...
  async obtenerUno(@Param('id') id: string) {
    ...
    if (numeroValido) {
      const registroEncontrado =
        await this.casaService.obtenerUnoPorId(numeroCasteado);
  ...
  actualizarUnoPorId(
    ...
    try {
      return this.casaService.actualizarUnoPorId(parametrosCuerpo, +id);
    } catch (e) {
      throw new NotFoundException(
        'Registro no encontrado, o error del servidor',
      );
    }
  }
  @Delete(':id')
  async eliminarUnoPorId(@Param('id') id: string) {
